...
const vec4 TRAIL_COLOR = vec4(0.502, 0.843, 0.710, 1.0); // theme mint
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (isFarEnough && isOnSeparateLine) {
        ...
        newColor = mix(newColor, TRAIL_COLOR_ACCENT, (1.0 - smoothstep(sdfTrail, -0.01, 0.001)) * TRAIL_INTENSITY);
        newColor = mix(newColor, TRAIL_COLOR, antialising(sdfTrail) * TRAIL_INTENSITY);
